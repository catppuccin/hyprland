...
1. Download the file with your desired flavour e.g. `mocha.lua` (to be found in the [release](https://github.com/catppuccin/hyprland/releases/latest) or after cloning the repository in `themes/`)
2. Copy it into your hyprland config e.g. `~/.config/hypr/`
3. Include the file at the top of your `hyprland.lua`
   - local catppuccin = require("mocha")
4. When using the non-alpha colors, use `catppuccin.COLOR` e.g. `catppuccin.base`
5. When using the alpha colors use something like `rgba(catppuccin.COLORAlpha<transparency>)` e.g. `rgba(catppuccin.surface0Alphaee)`

## 🙋 FAQ

-	Q: **_"Hyprland doesn't work with the colors"_**\
	A: Make sure you included the file in the right place and you are using `catppuccin.COLOR`
